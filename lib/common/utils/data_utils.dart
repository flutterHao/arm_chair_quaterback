...
class MyDateUtils {
  ...
  static bool isTomorrow(int tomorrowMs, {DateTime? nowDateTime}) {
    ...
    var tomorrowDateTime = getDateTimeByMs(tomorrowMs);
    // 比较目标日期的年月日部分是否等于明天的日期
    return tomorrowDateTime.year == tomorrow.year &&
  ...
  static bool isWeek(int ms, {bool isUtc = false, int? locMs}) {
    ...
    DateTime now0;
    if (locMs != null) {
      now0 = MyDateUtils.getDateTimeByMs(locMs, isUtc: isUtc);
    } else {
  ...
  /// Aug 1
  static String getEnMMDD(DateTime dateTime, {bool short = false}) {
    return "${getMonthEnName(dateTime, short: short)} ${dateTime.day}";
  }
