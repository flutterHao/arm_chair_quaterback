...
class TeamApi {
  ...
  static Future<MyTeamEntity> changeTeamPlayer(String? uuid1, String? uuid2) async {
    var json = await HttpUtil().post(Api.changeTeamPlayer, data: {"uuId1": uuid1 ?? "", "uuId2": uuid2 ?? ""});
  ...
  ///开启宝箱
  static Future<List<TrainingInfoAward>> opneBattleBox(int index, int playerId) async {
  ...
  static Future<int> buyTrainingBall(count) async {
    var json = await HttpUtil().post(Api.buyTrainingBall, data: {"buyCount": count});
    return json["num"];
  }

  static Future<List<TrainingInfoBuff>> chooseTactic(int tacticId, {int replaceTacticId = 0}) async {
    List list =
        await HttpUtil().post(Api.chooseTactic, data: {"tacticId": tacticId, "replaceTacticId": replaceTacticId});
  ...
  static Future dismissPlayer(String uuid, {int dismissType = 1}) async {
    var isDismiss = await HttpUtil().post(Api.dismissPlayer, data: {"uuids": uuid, "dismissType": dismissType});
    return isDismiss;
  }

  static Future<TeamPlayerUpStarVoEntity> getTeamPlayerUpStarVO(String uuid) async {
    var json = await HttpUtil().post(
      Api.getTeamPlayerUpStarVO,
      data: {"uuid": uuid},
    );
    TeamPlayerUpStarVoEntity teamEntity = TeamPlayerUpStarVoEntity.fromJson(json);
    return teamEntity;
  }
  ...
  static Future<PlayerCollectEntity> getPlayerCollect({int bookId = 101}) async {
    var json = await HttpUtil().post(Api.getTeamPlayerCollect, data: {"bookId": bookId});
    return PlayerCollectEntity.fromJson(json);
  }

  static Future composeTeamPlayer({required int playerId, int bookId = 101, int composeNum = 1}) async {
    var result = await HttpUtil()
        .post(Api.composeTeamPlayer, data: {"bookId": bookId, "composeNum": composeNum, "playerId": playerId});

    return result;
  }

  static Future destroyTeamPlayer({int bookId = 101, required int playerId}) async {
  ...
  static Future<List<Last5AvgEntity>> getNBAGameLogByTeamId(int teamId, String season) async {
    List list = await HttpUtil()
        .post(Api.getNBAGameLogByTeamId, data: {"season": season, "seasonType": "Regular%20Season", "teamId": teamId});
    return list.map((e) => Last5AvgEntity.fromJson(e)).toList();
  }

  /// 获取球员实力排行榜
  static Future<List<PlayerStrengthRankEntity>> getPlayerStrengthRank({int start = 0, int end = -1}) async {
    List json = await httpUtil.post(Api.getPlayerStrengthRank, data: {"start": start, "end": end});
